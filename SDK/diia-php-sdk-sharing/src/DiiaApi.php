...
class DiiaAPI
{
    ...
    public function getDeepinkSharingLink($branchId, $offerId, $returnLink, $requestId, $useDiiaId): string
    {
        ...
        if (isset($response['deeplink'])) {
            ...
            throw new Exception('Failed to retrieve deeplink');
        }
    }
    
    
    public function barcodeSharing($branchId, $barcode, $requestId, $useDiiaId): bool
    {
        $this->validateBranchId($branchId);
        $this->validateBarcode($barcode);
        
        $url = $this->diiaHost . '/api/v1/acquirers/document-request';
        ...
        var_dump($response); // Add for debugging
    ...
    {
        if (empty($branchId)) {
            throw new Exception('branchId is not provided');
        }
    ...
    {
        if (empty($offerId)) {
            throw new Exception('offerId is not provided');
        }
    ...
    {
        if (empty($offerName)) {
            throw new Exception('offerName is not provided');
        }
    ...
    private function validateBarcode($barcode): bool
    {
        if (!preg_match("/^\d+$/", $barcode)) {
            throw new Exception('Barcode must consist of digits');
        }
